...
final class WorkoutSessionViewModel {
    ...
    var splitDuration = 1          // total length of current phase (for progress ring)
    var totalElapsed = 0           // cumulative workout time in seconds
    ...
    func skipBackward() {
        ...
        case .interval(let i, let s):
            if i > 0 {
                ...
                // Back to beginning of first interval
                transition(to: .interval(exerciseIndex: 0, setIndex: 0))
            }
    ...
    private func tick() {
        totalElapsed += 1
        if splitTimeRemaining > 0 {
            splitTimeRemaining -= 1
        }
        if splitTimeRemaining == 0 {
            handlePhaseEnd()
        }
